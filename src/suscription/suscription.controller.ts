import { Controller, Post, Req, Headers, Body, BadRequestException, UseGuards } from '@nestjs/common';
...
async createCheckout(@Body() body: CreateCheckoutDto) {
  return this.subscriptionService.createCheckoutSession(body.providerId);
}
...
/*   @UseGuards(JwtAuthGuard)
  ...
  async createCheckout(@Req() req: Request) {
    const providerId = req.user['id']; // ✅ viene del JWT
    return this.subscriptionService.createCheckoutSession(providerId);
  } */
  @Post('webhook')
